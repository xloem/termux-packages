...
termux_step_make_install() {
	...
	if [ $TERMUX_ARCH = "arm" ]; then
		...
		cp -r ./out/ReleaseAndroidARM/dart-sdk $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/lib
		...
		cp -r ./out/ReleaseAndroidIA32/dart-sdk $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/lib
		...
		cp -r ./out/ReleaseAndroidARM64/dart-sdk $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/lib
		...
		cp -r ./out/ReleaseAndroidX64/dart-sdk $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/lib
	else
		termux_error_exit "Unsupported arch '$TERMUX_ARCH'"
	fi

	for file in $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/lib/dart-sdk/bin/*; do
		if [[ -f "$file" ]]; then
			echo -e "#!${TERMUX_PREFIX}/bin/sh\nexec $file  \"\$@\"" \
			        > $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/bin/$(basename $file)
			chmod +x $TERMUX_PKG_MASSAGEDIR/${TERMUX_PREFIX}/bin/$(basename $file)
		fi
	done
}
...
termux_step_post_make_install() {
	...
	install -Dm600 $TERMUX_PKG_BUILDER_DIR/dart-pub-bin.sh \
		$TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/etc/profile.d/dart-pub-bin.sh
}
